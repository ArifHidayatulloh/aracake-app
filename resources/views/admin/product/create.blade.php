...
@section('content')
    <div class="space-y-6">
        ...
        <x-header-page.header-page title="Tambah Produk" description="Buat produk baru di toko Anda." />

        <x-form.card>
            <form action="{{ route('admin.product.store') }}" method="POST" enctype="multipart/form-data"
                x-data="{
                    images: @js(old('images', [])), // Perbaikan utama: inisialisasi images dengan old data atau array kosong
                    ...
                    addImageField() {
                        ...
                        // Atur thumbnail jika ini adalah gambar pertama
                    ...
                    removeImageField(index) {
                        ...
                        // Sesuaikan indeks thumbnail
                        if (this.thumbnailIndex === index) {
                            this.thumbnailIndex = this.images.length > 0 ? 0 : null;
                    ...
                    init() {
                        ...
                        // Jika tidak ada data old('images'), tambahkan satu field gambar
                        if (this.images.length === 0) {
                            this.addImageField();
                        } else {
                            // Jika ada data old('images'), cari thumbnail yang sudah diset
                            const oldThumbnailIndex = this.images.findIndex(image => image.is_thumbnail === true);
                            if (oldThumbnailIndex !== -1) {
                                this.thumbnailIndex = oldThumbnailIndex;
                            } else if (this.images.length > 0) {
                                // Jika tidak ada thumbnail yang diset, jadikan gambar pertama sebagai thumbnail
                                this.setAsThumbnail(0);
                            }
                        }
                    }
                ...
                <div class="mb-8 p-6 bg-gray-50 rounded-lg border border-gray-200">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <x-form.input name="name" label="Nama Produk" :value="old('name')" :required="true"
                            :error="$errors->first('name')" />

                        <x-form.select2 label="Kategori" name="category_id" id="category_id" :options="$categories"
                            :selected="old('category_id')" placeholder="Pilih Kategori Produk"
                            :error="$errors->first('category_id')" :required="true" />
                    </div>

                    <x-form.input name="sku" label="SKU" :value="old('sku')" :required="true" :error="$errors->first('sku')" />

                    <div class="mt-5">
                        <x-form.markdown-editor name="description" label="Deskripsi" :error="$errors->first('description')"
                            rows="8" :value="old('description')" />
                    </div>

                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mt-6">
                        <x-form.input name="price" label="Harga" type="number" :value="old('price')" :required="true"
                            :error="$errors->first('price')" />
                        <x-form.input name="preparation_time_days" label="Waktu Pengerjaan (hari)" type="number"
                            :value="old('preparation_time_days')" :required="true" :error="$errors->first('preparation_time_days')" />
                    </div>
                ...
                <div class="mt-8 p-6 bg-gray-50 rounded-lg border border-gray-200">
                    ...
                    <template x-for="(image, index) in images" :key="index">
                        <div class="relative mb-6 p-4 border border-gray-200 rounded-lg bg-white shadow-sm">
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                <div>
                                    <label :for="'image_file_' + index"
                                        class="block text-sm font-medium text-gray-700 mb-2">
                                        File Gambar <span class="text-red-500">*</span>
                                    </label>
                                    <input type="file" :name="'images[' + index + '][file]'" :id="'image_file_' + index"
                                    ...
                                    {{-- Hilangkan preview gambar saat validasi gagal --}}
                                    <div x-show="image.preview" class="mt-4">
                                ...
                                <div>
                                    <label :for="'alt_text_' + index" class="block text-sm font-medium text-gray-700 mb-2">
                                    ...
                                    <div class="mt-4 flex items-center">
                                        ...
                                        <label :for="'is_thumbnail_' + index" class="ml-2 block text-sm text-gray-900">
